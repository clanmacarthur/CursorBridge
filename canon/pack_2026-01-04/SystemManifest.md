...
Source: Supabase PostgREST OpenAPI (introspected) + archive/docs/SYSTEM_MANIFEST.md where available
...
| control_pack_items | Join: packs â†” controls. | engine-level | read-only library | control_definitions, control_packs |
...
| mappings | Automation rules (valueâ†’relation, defaults). | engine-level | read-only library | - |
...
| profile_pack_map | Programme profiles â†’ default packs. | engine-level | read-only library | control_packs |
